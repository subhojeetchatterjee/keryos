...
def _styles() -> dict:
    ...
    s["title"] = ParagraphStyle(
        "k_title", fontName="Helvetica-Bold", fontSize=17,
        textColor=_NAVY, spaceBefore=0, spaceAfter=3, leading=20,
    )
    s["subtitle"] = ParagraphStyle(
        "k_subtitle", fontName="Helvetica", fontSize=10,
        textColor=_MUTED, spaceBefore=0, spaceAfter=4, leading=14,
    )
    s["ref"] = ParagraphStyle(
        "k_ref", fontName="Courier", fontSize=8.5,
        textColor=_TEAL, spaceBefore=0, spaceAfter=2,
    )
    s["section"] = ParagraphStyle(
        "k_section", fontName="Helvetica-Bold", fontSize=9.5,
        textColor=_NAVY, spaceBefore=14, spaceAfter=5,
        borderPadding=(0, 0, 3, 0),
    )
    s["body"] = ParagraphStyle(
        "k_body", fontName="Helvetica", fontSize=9,
        textColor=_TEXT, spaceBefore=2, spaceAfter=3, leading=13,
    )
    s["body_bold"] = ParagraphStyle(
        "k_body_bold", fontName="Helvetica-Bold", fontSize=9,
        textColor=_TEXT, spaceBefore=2, spaceAfter=3, leading=13,
    )
    s["caption"] = ParagraphStyle(
        "k_caption", fontName="Helvetica-Oblique", fontSize=7.5,
        textColor=_MUTED, spaceBefore=2, spaceAfter=6, leading=11,
    )
    s["bullet"] = ParagraphStyle(
        "k_bullet", fontName="Helvetica", fontSize=8.5,
        textColor=_TEXT, spaceBefore=1, spaceAfter=2,
        leftIndent=10, leading=12,
    )
    s["note"] = ParagraphStyle(
        "k_note", fontName="Helvetica-Oblique", fontSize=8,
        textColor=_MUTED, spaceBefore=2, spaceAfter=3, leading=11,
    )
    s["mono"] = ParagraphStyle(
        "k_mono", fontName="Courier", fontSize=7.5,
        textColor=_MUTED, spaceBefore=1, spaceAfter=1,
    )
    s["callout_status"] = ParagraphStyle(
        "k_cs", fontName="Helvetica-Bold", fontSize=10,
        spaceBefore=0, spaceAfter=3, leading=13,
    )
    s["callout_body"] = ParagraphStyle(
        "k_cb", fontName="Helvetica", fontSize=8.5,
        textColor=_TEXT, spaceBefore=1, spaceAfter=2, leading=12,
    )
    # ── New styles for enterprise redesign ──────────────────────────────────────
    s["cover_title"] = ParagraphStyle(
        "k_ct", fontName="Helvetica-Bold", fontSize=28,
        textColor=_WHITE, spaceBefore=0, spaceAfter=4, leading=32,
    )
    s["cover_sub"] = ParagraphStyle(
        "k_cs2", fontName="Helvetica", fontSize=11,
        textColor=_GRAY_D, spaceBefore=0, spaceAfter=0, leading=15,
    )
    s["section_num"] = ParagraphStyle(
        "k_sn", fontName="Helvetica-Bold", fontSize=10,
        textColor=_TEAL_BRIGHT, spaceBefore=0, spaceAfter=0,
    )
    s["section_title"] = ParagraphStyle(
        "k_st", fontName="Helvetica-Bold", fontSize=10,
        textColor=_NAVY, spaceBefore=0, spaceAfter=0,
    )
    s["kpi_val"] = ParagraphStyle(
        "k_kv", fontName="Helvetica-Bold", fontSize=18,
        textColor=_NAVY, spaceBefore=0, spaceAfter=1, leading=20,
    )
    s["kpi_lbl"] = ParagraphStyle(
        "k_kl", fontName="Helvetica-Bold", fontSize=7,
        textColor=_MUTED, spaceBefore=0, spaceAfter=2,
    )
    s["kpi_sub"] = ParagraphStyle(
        "k_ks", fontName="Helvetica", fontSize=7.5,
        textColor=_MUTED, spaceBefore=0, spaceAfter=0, leading=10,
    )
    s["exec_summary"] = ParagraphStyle(
        "k_ex", fontName="Helvetica", fontSize=9.5,
        textColor=_TEXT, spaceBefore=2, spaceAfter=4, leading=14,
    )
    s["img_label"] = ParagraphStyle(
        "k_il", fontName="Helvetica-Bold", fontSize=7.5,
        textColor=_WHITE, spaceBefore=0, spaceAfter=0,
    )
    s["img_caption"] = ParagraphStyle(
        "k_ic", fontName="Helvetica-Oblique", fontSize=7,
        textColor=_MUTED, spaceBefore=0, spaceAfter=0, leading=10,
    )
    return s
...
def _section_block(number: str, title: str, st: dict) -> list:
    ...
    return [
        ...
        Paragraph(
            f'<font color="#0e6e70"><b>{number} &nbsp;</b></font>'
            f'<font color="#0d2137"><b>{title}</b></font>',
            st["section"],
        ),
    ]
    ...
    for i, (label, value, sub, accent) in enumerate(metrics):
        row.append([
            Paragraph(label.upper(), ParagraphStyle(
                f"kl{i}", fontName="Helvetica-Bold", fontSize=7,
                textColor=accent, spaceAfter=2,
            )),
            Paragraph(f"<b>{value}</b>", ParagraphStyle(
                f"kv{i}", fontName="Helvetica-Bold", fontSize=18,
                textColor=_NAVY, spaceAfter=1, leading=20,
            )),
            Paragraph(sub, ParagraphStyle(
                f"ks{i}", fontName="Helvetica", fontSize=7.5,
...
def _ndvi_spectrum_bar(mean_ndvi: float, width: float = _CW) -> list:
    ...
    lbl_row = [
        Paragraph(label, ParagraphStyle(
            f"bl{i}", fontName="Helvetica", fontSize=6,
...
def _image_card(b64: str | None, label: str, caption: str,
                img_w: float, img_h: float, st: dict) -> Table:
...
def _ai_section_blocks(ai_assessment: dict, ai_narrative: str, st: dict) -> list:
    ...
    model_txt = ("Deterministic baseline  (Vertex AI not enabled)"
                 if is_fallback else "Claude 3.5 Sonnet  ·  Google Cloud Vertex AI")
    ...
    attr_tbl = Table([[
        Paragraph("AI-ASSISTED ASSESSMENT", ParagraphStyle(
            "ai_hd", fontName="Helvetica-Bold", fontSize=9,
            textColor=_WHITE, spaceAfter=0,
        )),
        Paragraph(model_txt, ParagraphStyle(
            "ai_md", fontName="Helvetica", fontSize=7.5,
            textColor=HexColor("#cffafe"), spaceAfter=0,
        )),
    ...
    exec_box = Table(
        [[Paragraph("EXECUTIVE SUMMARY", ParagraphStyle(
            "ai_el", fontName="Helvetica-Bold", fontSize=7,
            textColor=_TEAL_BRIGHT, spaceAfter=3,
        ))],
    ...
    if ins_int or tech_anal:
        ...
        right_body: list = []
        if tech_anal:
            left_body.append(Paragraph("TECHNICAL ANALYSIS", ParagraphStyle(
                "ai_tl", fontName="Helvetica-Bold", fontSize=7, textColor=_MUTED, spaceAfter=3,
            )))
            left_body.append(Paragraph(tech_anal, st["body"]))
        if ins_int:
            right_body.append(Paragraph("INSURANCE INTERPRETATION", ParagraphStyle(
                "ai_il", fontName="Helvetica-Bold", fontSize=7, textColor=_AMBER, spaceAfter=3,
            )))
    ...
    if conf_exp:
        items.append(Paragraph(
            f"<b>Confidence basis:</b>  {conf_exp}", st["note"],
        ))
...
def _draw_cover_page(report: dict, ref_id: str, gen_display: str):
    ...
    def _cb(canvas, doc) -> None:
        ...
        canvas.drawRightString(w - 0.75 * inch, h - 0.25 * inch,
                               "Satellite Claim Verification Intelligence")
        ...
        canvas.drawString(0.75 * inch, ty - 0.88 * inch,
                          "Prevented-Sowing Agricultural Insurance Assessment")
        ...
        health_class = ndvi_interp.get("health_class", "moderate")
        verdict_map = {
            "healthy":  ("VEGETATION DETECTED",   "CLAIM NOT SUPPORTED BY EVIDENCE",
                         _GREEN_D, _GREEN_L),
            "moderate": ("BORDERLINE SIGNAL",      "FIELD VERIFICATION REQUIRED",
                         _AMBER_D, _AMBER_L),
            "stressed": ("NO CROP EMERGENCE",      "CLAIM SUPPORTED BY SATELLITE EVIDENCE",
                         _RED_D, _RED_L),
        }
        ...
        ndvi_line = (f"Composite NDVI: {mean_val:.3f}  ·  {health_label}"
                     if mean_val is not None else health_label)
        ...
        canvas.setFillColor(_GRAY_D)
        canvas.setFont("Helvetica", 6.5)
        canvas.drawString(0.75 * inch, 0.19 * inch,
...
def _draw_chrome_summary(canvas, doc) -> None:
    ...
    canvas.drawRightString(w - 0.6 * inch, 0.14 * inch,
...
def generate_verification_pdf(report: dict, polygon_coords: dict | None) -> bytes:  # noqa: ARG001
    ...
    story.append(_kpi_row([
        ...
        ("Confidence",      conf_kpi,   "Data quality assessment",                        {"High": _GREEN, "Medium": _AMBER, "Low": _RED}.get(conf_label, _MUTED)),
        ("Sat. Passes",     passes_kpi, f"{pooled.get('totalPixels', 0):,} valid pixels", HexColor("#7c3aed")),
    ]))
    ...
    centroid_str = (
        f"{centroid['lat']:.5f}° N, {centroid['lon']:.5f}° E" if centroid else "N/A"
    )
    ...
    gap = _CW - 2 * img_w
    tc_card   = _image_card(best.get("png_b64"),       "TRUE COLOUR  RGB",
                            f"Sentinel-2 B04/B03/B02 · {report.get('best_date')}",
                            img_w, img_h, st)
    swir_card = _image_card(best.get("swir_png_b64"),  "FALSE COLOUR  SWIR",
                            f"Sentinel-2 B11/B08/B04 · {report.get('best_date')}",
                            img_w, img_h, st)
    img_layout = Table([[tc_card, Spacer(gap, 1), swir_card]],
    ...
    if stats_data is not None:
        ...
        ndvi_rows = [
            ...
            ["IQR (P25–P75)",
             f"{float(_p25):.3f} – {float(_p75):.3f}"
             if _p25 is not None and _p75 is not None else "N/A",
             "Middle 50% of pixel distribution"],
        ]
        story.append(Table(ndvi_rows, colWidths=[1.5 * inch, 0.9 * inch, None],
                           style=_tbl_style(header_bg=_TEAL)))
        story.append(Spacer(1, 6))
    else:
    ...
    if pooled:
        comp_rows = [
            ...
            ["Composite Mean NDVI",      f"{pooled['mean']:.3f}",
             "Weighted average — all valid passes"],
            ["Pooled Standard Deviation", f"{pooled['stDev']:.3f}",
             "Cross-temporal within-field variability"],
            ["Valid Satellite Passes",    str(pooled["passes"]),
             "Distinct cloud-free acquisition epochs"],
            ["Total Valid Pixels",        f"{pooled['totalPixels']:,}",
             "Clear-sky pixels in composite"],
        ]
        story.append(Table(comp_rows, colWidths=[2.0 * inch, 1.0 * inch, None],
                           style=_tbl_style(header_bg=_NAVY)))
    else:
    ...
    ai_val_str  = (("Validated" if confidence.get("ai_validated") else "Flagged")
                   if confidence.get("ai_validated") is not None else "Not enabled")
    ai_conf_str = (f"{confidence['ai_confidence']:.0%}"
                   if confidence.get("ai_confidence") is not None else "N/A")
    ...
    story.append(Table(tech_rows, colWidths=[2.2 * inch, None],
                       style=_tbl_style(header_bg=_TEAL)))
    story.append(Spacer(1, 4))
...
def generate_summary_pdf(report: dict) -> bytes:
    ...
    if ndvi_interp:
        health_class = ndvi_interp.get("health_class", "moderate")
        verdict_map = {
            "healthy":  (_GREEN_L, _GREEN_D,  "CLAIM NOT SUPPORTED",
                         "Vegetation detected within sowing window"),
            "moderate": (_AMBER_L, _AMBER_D,  "INCONCLUSIVE — FIELD VERIFICATION",
                         "Borderline NDVI signal"),
            "stressed": (_RED_L,   _RED_D,    "CLAIM SUPPORTED BY SATELLITE EVIDENCE",
                         "No crop emergence detected"),
        }
        bg, fg, verdict_text, verdict_sub = verdict_map.get(
        ...
        verdict_tbl = Table([[
            [Paragraph(f'<b>VERDICT</b>', ParagraphStyle(
             ...
             Paragraph(f'<b>{verdict_text}</b>', ParagraphStyle(
                 "vt", fontName="Helvetica-Bold", fontSize=12,
                 textColor=fg, spaceAfter=1,
             )),
             Paragraph(verdict_sub, ParagraphStyle(
                 "vs", fontName="Helvetica", fontSize=8.5, textColor=_TEXT,
             ))],
        ]], colWidths=[None])
    ...
    story.append(_kpi_row([
        ("Composite NDVI",  ndvi_kpi,   ndvi_interp.get("health_label", ""), _TEAL_BRIGHT),
        ("Cloud Score",     cloud_kpi,  "Best scene clarity",                 HexColor("#2563eb")),
        ("Confidence",      conf_kpi,   "Data quality",
         {"High": _GREEN, "Medium": _AMBER, "Low": _RED}.get(conf_label, _MUTED)),
        ("Sat. Passes",     passes_kpi, f"{pooled.get('totalPixels', 0):,} px",
         HexColor("#7c3aed")),
    ], width=cw_letter))
    story.append(Spacer(1, 8))
    ...
    facts = [
        ...
        ["AOI Area",               area_str],
        ["Composite Mean NDVI",
         f"{pooled['mean']:.3f}" if pooled.get("mean") is not None else "N/A"],
        ["Vegetation Class",       ndvi_interp.get("health_label", "N/A")],
        ["Valid Pixels",
         f"{pooled['totalPixels']:,}" if pooled.get("totalPixels") else "N/A"],
    ]
    facts_tbl = Table(facts, colWidths=[1.6 * inch, 2.3 * inch],
                      style=_tbl_style(header_bg=_NAVY))
    ...
    exec_text = (ai_assess.get("executive_summary") if isinstance(ai_assess, dict) else None) \
                or report.get("ai_narrative", "")
    if exec_text:
        is_fallback = ai_assess.get("fallback", True) if isinstance(ai_assess, dict) else True
        model_lbl = ("Deterministic baseline" if is_fallback
                     else "Claude 3.5 Sonnet · Vertex AI")
        ai_box = Table([
            [Paragraph(
                f'<font color="#0d9488"><b>AI ASSESSMENT</b></font>'
                f'<font color="#64748b">  &nbsp;·&nbsp;  {model_lbl}</font>',
                ParagraphStyle("ahb", fontName="Helvetica-Bold", fontSize=8.5,
...
def _draw_chrome_appendix(canvas, doc) -> None:
    ...
    canvas.setFillColor(_GRAY_D)
    canvas.setFont("Helvetica", 6)
    canvas.drawString(0.7 * inch, 0.14 * inch,
                      "For academic / evaluation purposes — Keryos v0.1")
    canvas.drawRightString(w - 0.7 * inch, 0.14 * inch,
                           datetime.now().strftime("%Y-%m-%d %H:%M UTC"))
    canvas.restoreState()
...
def generate_appendix_pdf(report: dict) -> bytes:
    ...
    cover_rows = [
        ...
        ["Best Acquisition", report.get("best_date", "N/A"),
         "Crop", report.get("crop_type", "N/A").title()],
        ["Claim Period",
         f"{report.get('date_from')} – {report.get('date_to')}",
         "Keryos Version", proc.get("keryos_version", "0.1")],
    ]
    cv_tbl = Table(cover_rows,
                   colWidths=[1.2 * inch, 2.15 * inch, 1.2 * inch, 2.15 * inch])
    ...
    band_tbl = Table(band_rows, colWidths=[0.55 * inch, 1.1 * inch, 1.1 * inch,
                                           1.0 * inch, None])
    ...
    story.append(Paragraph(
        ...
        "to Keryos v" + proc.get("keryos_version", "0.1") +
        " will reproduce an identical analysis.",
        st["body"],
    ))
    story.append(Spacer(1, 4))
    ...
    if params or thresh or ndvi_c:
        ...
        story.append(Paragraph(
            "Detailed parameters not available — report was generated by an "
            "older version of Keryos.",
            st["note"],
        ))
    story.append(Spacer(1, 6))
    ...
    for rq in RESEARCH_QUESTIONS:
        ...
        rq_box = Table(
            [[Paragraph(f'<b><font color="#0e6e70">{rq["number"]}</font></b>', st["body"]),
              rq_inner]],
            colWidths=[0.55 * inch, None],
        )
        rq_box.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, -1), _GRAY_L),
    ...
    alt_tbl = Table(alt_rows,
                    colWidths=[1.5 * inch, 0.55 * inch, 0.65 * inch,
                               0.65 * inch, 0.65 * inch, None])
